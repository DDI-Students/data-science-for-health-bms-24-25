...
The dataset for this assignment can be found as a csv file.
...
**Warm up 1:** Verify that the dataset has loaded into the Environment.
How many observations are in the dataset?
There are 240 observations in the dataset.
...
This means that there are some records where the full information is not available.
...
```{r plastic_waste_per_cap-dens-color-fill-alpha-update}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     color = continent, 
                     fill = continent)) +
...
Here the color and fill settings define which data we want to use; this is plot agnostic. However the alpha level is specific to this type of plot so cannot be coded in the same way.
...
Violin plots reveal the spread of the data (or the density). However in the box-plot the median, 1st quartile and 3rd quartile are evident whilst they are not in the violin-plots.
...
```{r plastic_waste_mismanaged_scatter}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
...
The relationship is somewhat linear. There are a few outliers to the this (particularly the point on the far right and the ones closer to the main 'chunk')
...
```{r plastic_waste_mismanaged_scatter_coloured}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
...
```{r plastic_waste_mismanaged_scatter_coloured_facet}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     y = mismanaged_plastic_waste_per_cap)) +
  geom_point() +
...
Here we can see a fairly linear trend in most of the groupings now. Europe however is more of a homogenous 'blob' of data.
